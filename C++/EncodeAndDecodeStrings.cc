...
// Note:
// 1. The string may contain any possible characters out of 256 valid ascii characters. Your algorithm
// should be generalized enough to work on any possible characters.
// 2. Do not use class member/global/static variables to store states. Your encode and decode algorithms
// should be stateless.
...
public:
    ...
    string encode(vector<string>& strs) {
        return accumulate(strs.begin(), strs.end(), string{},
            [](string& acc, string& s){ return acc + to_string(s.size()) + '|' + s;});
    }
